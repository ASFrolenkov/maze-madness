...
@Slf4j
public class GameEventService implements GameEvents {

    private final SocketIOServer server;
    @Autowired
    private ConcurrentHashMap<String, PlayerDto> connectedPlayers;
    ...
    public void onMoveReceived(SocketIOClient client, String moveData) {
        ...
        try {
            if (connectedPlayers.get(client.getSessionId().toString()) != null){
                ...
                currentPlayer.setX(objectMapper.readValue(moveData, MoveDto.class).getX());
                currentPlayer.setY(objectMapper.readValue(moveData, MoveDto.class).getY());
